 ...
 * @author matej
 */
public class ModelInterpretability {
    ...
    //testing for different memory range
 ...
 //start function   
       ...
       /* SettingsReader initSettings=new SettingsReader();
        ...
        //read the rules obtained from first attribute set
         // String input1="C:\\Users\\matej\\Documents\\Redescription mining with CLUS\\unctad.out";
          ...
          if(view1 ==0 || appset.posVal==0)
                ruleReaders.get(view1).extractRules(input1,fid,datJInit,appset,0);
          else if(appset.posVal>0)
                ruleReaders.get(view1).extractRulesMod(input1,fid,datJInit,appset,0);
       ...
       if(view2==0 || appset.posVal==0)
       ruleReaders.get(view2).extractRules(input1,fid,datJInit,appset,0);
       else if(appset.posVal>0)
            ruleReaders.get(view2).extractRulesMod(input1,fid,datJInit,appset,0);
...
//put outside function
       ...
       if((leftSide==1 && (endIndexRR1-oldIndexRR1)>z*appset.numTargets) || (rightSide==1 && (endIndexRR-oldIndexRR)>z*appset.numTargets)){
           if(leftSide == 1 && view2==0 || rightSide == 1 && view1==0 || appset.posVal==0)
                 ItRules.extractRules(input,fid,datJ,appset,0);
           else if(appset.posVal>0)
                 ItRules.extractRulesMod(input,fid,datJ,appset,0);
            ItRules.setSize();
            if(appset.numSupplementTrees>0){
                if(appset.SupplementPredictiveTreeType == 0){
                    if(leftSide == 1 && view2==0 || rightSide == 1 && view1==0 || appset.posVal==0)
                     ItRulesF.extractRules(inputF,fid,datJ,appset,0);
                        ...
                        ItRulesF.extractRulesMod(inputF,fid,datJ,appset,0);
                }
                else {
                     if(leftSide == 1 && view2==0 || rightSide == 1 && view1==0 || appset.posVal==0)
                        ItRulesF.extractRules(inputF,fid,datJ,appset,1);
                     else if(appset.posVal>0)
                           ItRulesF.extractRulesMod(inputF,fid,datJ,appset,1);
                }
                ItRulesF.setSize();
            }
       }
       ...
       if((leftSide1==1 && (endIndexRR1-oldIndexRR1)>z*appset.numTargets) || (rightSide1==1 && (endIndexRR-oldIndexRR)>z*appset.numTargets)){
          if(leftSide == 1 && view2==0 || rightSide == 1 && view1==0 || appset.posVal == 0)
           ItRules1.extractRules(input,fid,datJ,appset,0);
                ...
                ItRules1.extractRulesMod(input,fid,datJ,appset,0);
        ItRules1.setSize();
        if(appset.numSupplementTrees>0){
            if(appset.SupplementPredictiveTreeType == 0){
              if(leftSide == 1 && view2==0 || rightSide == 1 && view1==0 || appset.posVal == 0)
                ItRulesF1.extractRules(inputF,fid,datJ,appset,0);
                   ...
                   ItRulesF1.extractRulesMod(inputF,fid,datJ,appset,0);
            }
            else{
                if(leftSide == 1 && view2==0 || rightSide == 1 && view1==0 || appset.posVal == 0)
                  ItRulesF1.extractRules(inputF,fid,datJ,appset,1);
                else 
                    ItRulesF1.extractRulesMod(inputF,fid,datJ,appset,1);  
            }
             ItRulesF1.setSize();
        }
       }
      ...
      //add to second function (generalize)
       if(appset.optimizationType==0){ 
        if(appset.useJoin){
      ...
      /* if(rs.redescriptions.size()==2)
        ...
        //third function for a given view append rules
        ...
        for(int z=0;z<numBins/*percentage.length-1*/;z++){
            ...
            if(z==0){//should create network from redescriptions!
               if(appset.useNC.size()>nws && appset.networkInit==false){ 
                ...
                //oldRIndex[0] = ruleReaders.get(nws).newRuleIndex;
            }
      ...
      rs.redescriptions.addAll(t);
       ...
       // double a[] = {0,0};
       // rs.writeToFile(appset.outFolderPath+"\\"+appset.outputName+(1)+"intermid"+(view1)+"_"+view2+".rr", datJ, fid, startTime,0,appset, 0,a,oom);
