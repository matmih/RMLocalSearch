 ...
 * @author matmih
 */
public class MonteCarloSimulationsParallel {
   ...
   Utilities ut = new Utilities();      
     ...
     if(appset.system.equals("windows")){ 
      ...
      for(int i=0;i<appset.parameters.size();i++){
          ...
          appset.minSupport= appset.parameters.get(i).get(1).intValue();
           ...
           //Add MC part
           System.out.println("MC starting!");
           RedescriptionSet mcTmp = new RedescriptionSet();
           ArrayList<Double> minScores = new ArrayList<>();
           MersenneTwister mcGenerator = new MersenneTwister();
           int maxTrials = 300, maxIter = 10000;
           ExecutorService executor = Executors.newFixedThreadPool(15);
           ArrayList<Integer> tmp = new ArrayList<>(appset.numRetRed);
           HashSet<Integer> ti = new HashSet<>(appset.numRetRed);
           List<Callable<Double>> tasks = new ArrayList<>(maxIter);
           
           for(int mIterTrials = 0;mIterTrials<maxTrials;mIterTrials++){
               ArrayList<Double> scores = new ArrayList<>(maxIter);
               ...
               for(int mIter = 0; mIter<maxIter; mIter++){
                   ...
                   for(int k=0;k<appset.numRetRed;k++){
                        int index = mcGenerator.nextInt(rs.redescriptions.size());
           
                         while(ti.contains(index)){
                                   index = mcGenerator.nextInt(rs.redescriptions.size());
                           }
                   ...
                   tasks.add(new MCRun(appset.numRetRed,tmp,rs,appset,fid,datJ));
                   ti.clear(); tmp = new ArrayList<>();
               ...
               try{
                   List<Future<Double>> values = executor.invokeAll(tasks);
                   executor.shutdown();
               ...
               for(int ii=1;ii<scores.size();ii++)
                    if(scores.get(ii)<min)
                        min = scores.get(ii);
               
               minScores.add(min);
               System.out.println("min: "+min);
               System.out.println("MC trial: "+mIterTrials);
               System.out.println("duration of a trial: "+(end-start)+" ns.");
               
               ti.clear();  tasks.clear();
     ...
     for(int fit=0;fit<resSets.size();fit++){
      ...
      System.out.println("Results score: "+ResultsScore);
       ...
       double averageEstimator  = 0.0;
        //ovdje isto ukljuceno fiksno rjesenje x^Kapa
        for(int ii=0;ii<minScores.size();ii++)
            averageEstimator+=(ResultsScore - minScores.get(ii));
        averageEstimator/=maxTrials;
        
        double varEst = 0.0;
        //ovo nije OK, treba za zadano rj. x^Kapa pronaci ocjenu do optimalnog
        for(int ii=0;ii<minScores.size();ii++)
            varEst += Math.pow((minScores.get(ii)-averageEstimator),2);
         varEst*=(1.0/(maxTrials - 1));
         
         double sd = Math.sqrt(varEst);
         double zAlpha = 2.58; //99% confidence interval
         
         System.out.println("averageEst: "+averageEstimator+" sd*alph/sqrt(mt): "+(zAlpha*sd/Math.sqrt(maxTrials)));
         System.out.println("99% confidence interval: ["+0+", "+(averageEstimator+zAlpha*sd/Math.sqrt(maxTrials))+"]");
         System.out.println("99% probability that the difference of obtained from optimal is <=: "+(averageEstimator+(zAlpha*sd/Math.sqrt(maxTrials))));
